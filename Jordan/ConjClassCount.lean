...
alias Equiv.Perm.disjoint_iff_support_disjoint := Equiv.Perm.disjoint_iff_disjoint_support
...
theorem List.exists_pw_disjoint_with_card [Fintype α] [DecidableEq α]
...
theorem Set.ncard_filter_eq_count {ι κ : Type*} [DecidableEq ι] [DecidableEq κ]
  ...
  · simp
  · suffices ({x ∈ insert a s | f x = k} : Set _) =
      if f a = k then insert a ({x ∈ s | f x = k} : Set _) else ({x ∈ s | f x = k} : Set _) by
      rw [this]
      simp only [Finset.insert_val, Finset.mem_val, Multiset.mem_map, Multiset.mem_ndinsert, exists_eq_or_imp, Multiset.ndinsert_of_notMem has]
      ...
      · rw [if_pos h, if_pos h.symm]
        rw [Set.ncard_insert_of_notMem ?_ ?_]
...
alias Equiv.Perm.mem_support_of_commute := Equiv.Perm.mem_support_iff_of_commute
...
alias Equiv.Perm.commute_of_mem_cycleFactorsFinset_iff :=
...
theorem Equiv.Perm.ofPartition_coe_apply' {ι : Type _} [DecidableEq ι]
  ...
  -- empty
  simp only [Finset.noncommProd_empty, Equiv.Perm.coe_one, id, Finset.not_mem_empty, if_false]
  rw [dif_neg id]
  · -- insert
...
theorem Equiv.Perm.of_partition_card {ι : Type*} [Fintype ι] [DecidableEq ι] (p : α → ι) :
  ...
  suffices ∀ x : Equiv.Perm α, (x ∈ MulAction.stabilizer (Equiv.Perm α) p ↔ p ∘ x = p) by
    simp_rw [← this]
    suffices Fintype.card {f | f ∈ MulAction.stabilizer (Equiv.Perm α) p} = Fintype.card (MulAction.stabilizer (Equiv.Perm α) p) by
  ...
  · intro x
    ...
    suffices ∀ x : Equiv.Perm α, (x • p = p ↔ p ∘ x = p) by
      simp_rw [this]
    intro x
...
theorem _root_.Equiv.Perm.Basis.Kf_apply'' (a : Equiv.Perm.Basis g)
  ...
  suffices hdc : (d : Equiv.Perm α).Disjoint (e c : Equiv.Perm α) by
    ...
    exact (Equiv.Perm.mem_cycleFactorsFinset_iff.mp (e c).prop).1
...
theorem haK5 (τ : Equiv.Perm g.cycleFactorsFinset) (x : α)
  ...
  simp only [Function.extend_apply' _ _ _ hx, id]
 -/
...
theorem _root_.Equiv.Perm.Basis.Kf_factorsThrough (a : Equiv.Perm.Basis g)
  ...
  suffices hcd : c = d by
    ...
    rw [g.zpow_eq_zpow_on_iff,
      ← Equiv.Perm.cycle_is_cycleOf (a := a (e d)) (a.mem_support_self _) (e d).prop] at He
    rw [g.zpow_eq_zpow_on_iff,
      ← Equiv.Perm.cycle_is_cycleOf (a := a (e' d)) (a.mem_support_self _) (e' d).prop, ← hee' d]
    exact He
    · rw [← Equiv.Perm.mem_support, ← Equiv.Perm.cycleOf_mem_cycleFactorsFinset_iff,
        ← Equiv.Perm.cycle_is_cycleOf (a := a (e' d)) (a.mem_support_self _) (e' d).prop]
      exact (e' d).prop
    · rw [← Equiv.Perm.mem_support, ← Equiv.Perm.cycleOf_mem_cycleFactorsFinset_iff,
        ← Equiv.Perm.cycle_is_cycleOf (a := a (e d)) (a.mem_support_self _) (e d).prop]
      exact (e d).prop
...
theorem k_apply (a : Equiv.Perm.Basis g)
  ...
  · intro c; simp only [← hτ c, Equiv.Perm.coe_one, id]
...
theorem mem_support_iff_exists_Kf (a : Equiv.Perm.Basis g) (x : α) :
  ...
  · intro hx
    ...
    use ⟨g.cycleOf x, hx⟩
    simp only [Kf_def, Equiv.Perm.coe_one, id]
    let ha := a.mem_support_self ⟨g.cycleOf x, hx⟩
    simp only [Subtype.coe_mk, Equiv.Perm.mem_support_cycleOf_iff] at ha
    obtain ⟨i, hi⟩ := ha.1.symm
    exact ⟨i, hi.symm⟩
  · rintro ⟨c, i, rfl⟩
    simp only [Kf_def, Equiv.Perm.zpow_apply_mem_support, Equiv.Perm.coe_one, id]
    apply Equiv.Perm.mem_cycleFactorsFinset_support_le c.prop
    apply a.mem_support_self
...
theorem k_one : k a (1 : Equiv.Perm g.cycleFactorsFinset) = id := by
  ...
  · simp only [mem_support_iff_exists_Kf a] at hx
    ...
    rfl
    intro c; rfl
  simp only [id, k_apply_of_not_mem_support x hx]
...
theorem k_bij {τ : Equiv.Perm g.cycleFactorsFinset}
  ...
  rw [k_mul]; rw [mul_inv_cancel]; rw [k_one]
  exact hτ
...
def Iφ : Subgroup (Equiv.Perm g.cycleFactorsFinset) where
  ...
  one_mem' := by
    simp only [Set.mem_setOf_eq, Equiv.Perm.coe_one, id, eq_self_iff_true, imp_true_iff]
  mul_mem' := by
    intro σ τ hσ hτ
...
def φ' (a : Equiv.Perm.Basis g) :
  ...
  map_one' := by
    ...
    simp
  map_mul' σ τ := by
    simp_rw [Subgroup.coe_mul, MulMemClass.mk_mul_mk, Subtype.mk.injEq, ← ConjAct.toConjAct_mul]
    apply congr_arg
    ext x
...
/- theorem hφ'_is_rightInverse (τ : Iφ g) :
  ...
  dsimp only [id_eq, OneHom.toFun_eq_coe, MonoidHom.coe_mk, OneHom.coe_mk,
    ConjAct.ofConjAct_toConjAct, Equiv.Perm.coe_mul, Function.comp_apply,
    Finset.coe_sort_coe]
  simp only [← Equiv.trans_apply, ← Equiv.Perm.mul_def, ← mul_assoc]
  apply Equiv.Perm.congr_fun
  rw [mul_inv_eq_iff_eq_mul]
  ext y
...
theorem φ'_support_le (τ : Iφ g) :
  ...
  intro hx' hx
  apply hx'
  rw [← Equiv.Perm.notMem_support] at hx
...
theorem Iφ_eq_range : Iφ g = (φ g).range := by
  ...
  constructor
  · intro hτ
    exact ⟨(φ' a) ⟨τ, hτ⟩, hφ'_is_rightInverse ⟨τ, hτ⟩⟩
  · rw [mem_Iφ_iff]
...
theorem hφ_range_card' :
  ...
  rw [← Nat.card_eq_fintype_card, ← hφ_range']
  rfl
...
theorem hφ_range_card :
  ...
  suffices hlc :
    ∀ n, Fintype.card {a : g.cycleFactorsFinset | fsc a = n} = g.cycleType.count ↑n by
    suffices hl_lt : ∀ i ∈ g.cycleType, i < Fintype.card α + 1 by
      simp_rw [hlc]
      ...
      · intro i hi
        ...
        rw [Multiset.count_eq_zero_of_notMem hi.2]
  ...
  let u : ({x : g.cycleFactorsFinset | fsc x = ⟨i, hi⟩} : Set _) →
      ({x ∈ g.cycleFactorsFinset | (Finset.card ∘ Equiv.Perm.support) x = i} : Set _) :=
    fun ⟨⟨x, hx⟩, hx'⟩ => ⟨x, by
  ...
  have : Function.Bijective u := by
    ...
    · rintro ⟨⟨x, hx⟩, hx'⟩ ⟨⟨y, hy⟩, hy'⟩ h
      simp only [Function.comp_apply, Set.coe_setOf, Set.mem_setOf_eq, Subtype.mk.injEq, u] at h
      simp only [h, Set.coe_setOf, Set.mem_setOf_eq, Subtype.mk.injEq]
    rintro ⟨x, hx⟩
...
lemma θAux_apply_of_mem_fixedPoints {k} {v} {x}
    ...
    θAux g k v x = Equiv.Perm.ofSubtype k x := by
  rw [θAux, dif_neg]
  rw [Equiv.Perm.cycleOf_mem_cycleFactorsFinset_iff, Equiv.Perm.notMem_support]
  exact hx
...
lemma θAux_cycleOf_apply_eq {k} {v} {x} :
  ...
  · rw [θAux_apply_of_cycleOf_eq ⟨g.cycleOf x, hx⟩ rfl]
    ...
    rw [← hm]
    simp only [Equiv.Perm.cycleOf_zpow_apply_self, Equiv.Perm.cycleOf_self_apply_zpow]
  · rw [g.cycleOf_mem_cycleFactorsFinset_iff, Equiv.Perm.notMem_support] at hx
...
lemma θAux_mul
  ...
  · rw [θAux_apply_of_cycleOf_eq ⟨g.cycleOf x, hx⟩ (θAux_apply_of_cycleOf_eq_mem ⟨_, hx⟩ rfl),
      ...
      θAux_apply_of_cycleOf_eq ⟨g.cycleOf x, hx⟩ rfl]
    · simp
  · have hx' : g.cycleOf (θAux g k v x) ∉ g.cycleFactorsFinset := by
...
lemma θAux_inv (k) (v) :
  ...
  intro x
  rw [θAux_mul]
  simp only [inv_mul_cancel, θAux_one]
...
lemma range_ψAux2_le_fixingSubgroup_fixedPoints (g : Equiv.Perm α) :
  ...
  simp only [← Equiv.Perm.notMem_support, not_imp_not]
  intro hx
...
def ψAux1comm2 (g : Equiv.Perm α)
  ...
  · right
    ...
    · simp only [mem_fixingSubgroup_iff, Equiv.Perm.smul_def] at this
      apply this
      simp only [Function.mem_fixedPoints, Function.IsFixedPt, ← Equiv.Perm.notMem_support]
      exact ha
...
theorem hψ_1 (uv) (x : α) (hx : x ∈ MulAction.fixedBy _ α g) :
  ...
  simp only [dif_pos hc, MulAction.mem_stabilizer_iff, Equiv.Perm.smul_def,
    ← Equiv.Perm.notMem_support, ← hm]
  simp only [MulAction.mem_fixedBy, Equiv.Perm.smul_def, ← Equiv.Perm.notMem_support] at hx
  intro hx'
  apply hx
...
theorem hθ_single (c : g.cycleFactorsFinset) :
  ...
  · simp only [hθ_1 _ x hx, Equiv.Perm.coe_one, id_eq]
    apply symm
    rw [← Equiv.Perm.notMem_support]
    simp only [Function.mem_fixedPoints, Function.IsFixedPt, ← Equiv.Perm.notMem_support] at hx
    intro hx'
    apply hx
  ...
  suffices hx' : g.cycleOf x ∈ g.cycleFactorsFinset by
    ...
    · rw [Pi.mulSingle_eq_of_ne' hc]
      ...
      apply hc
      rw [← Subtype.coe_inj]
      exact Equiv.Perm.cycle_is_cycleOf hxc c.prop
  rw [← Equiv.Perm.cycleOf_ne_one_iff_mem, ne_eq, Equiv.Perm.cycleOf_eq_one_iff]
  simpa [Function.mem_fixedPoints_iff] using hx
...
theorem hψ_2 (uv) (x : α) (c : Equiv.Perm α) (hc : c ∈ g.cycleFactorsFinset)
  ...
  · intro hs uv x c hc hx
    ...
    rw [Equiv.Perm.ofSubtype_apply_of_not_mem]
    simp only [MulAction.mem_fixedBy, Equiv.Perm.smul_def]
    rw [← Ne, ← g.isCycle_cycleOf_iff, ← hx]
    exact hc.1
...
/- theorem hψ_inj : Function.Injective (ψ g) := by
  ...
  · ext c hc x
    ...
    · obtain ⟨m, hm⟩ := Subgroup.mem_zpowers_iff.mp (uv.snd c hc).prop
      ...
      · rw [← Equiv.Perm.notMem_support]; intro hx'
...
theorem hθ_injective (g : Equiv.Perm α) : Function.Injective (θ g) := by
  ...
  · ext c x
    ...
    · simp only [Equiv.Perm.cycleOf_eq_one_iff, ← Equiv.Perm.notMem_support] at hx
      simp only [Pi.one_apply, OneMemClass.coe_one, Equiv.Perm.coe_one, id_eq]
      obtain ⟨m, hm⟩ := (v c).prop
      rw [← hm]
      dsimp
      rw [← Equiv.Perm.notMem_support]
      intro hx'
      suffices ¬ x ∈ Equiv.Perm.support c by
        apply this
...
/- theorem hφ_ker_eq_ψ_range (z : Equiv.Perm α) :
  ...
  · intro Hz
    have hu : ∀ x : α,
      ...
      intro x
      simp only [MulAction.mem_fixedBy, Equiv.Perm.smul_def]
      simp only [← Equiv.Perm.notMem_support]
      rw [not_iff_not]
      constructor
      · intro hx
    ...
    · rw [Equiv.Perm.notMem_support, ← Equiv.Perm.smul_def, ← MulAction.mem_fixedBy] at hx
      rw [hψ_1 ⟨u, v⟩ x hx]
      simp only [Subtype.coe_mk, Equiv.Perm.subtypePerm_apply]
  · rintro ⟨⟨u, v⟩, h⟩
    intro c hc
    ...
    · ext x
      ...
      · rw [Equiv.Perm.ofSubtype_apply_of_mem]
        ...
        exact hx
      · rw [Equiv.Perm.ofSubtype_apply_of_not_mem]
        apply symm; rw [← Equiv.Perm.notMem_support]
    ...
    · intro x
      ...
      · intro hzx
        ...
        · exfalso
          ...
          simp only [Equiv.Perm.notMem_support, ← Equiv.Perm.smul_def,
...
theorem hφ_ker_eq_θ_range (z : Equiv.Perm α) :
  ...
  · rw [hφ_mem_ker_iff, Equiv.Perm.IsCycle.forall_commute_iff, Set.mem_range]
    intro Hz
    have hu : ∀ x : α,
      z x ∈ Function.fixedPoints g ↔
        x ∈ Function.fixedPoints g :=  by
      intro x
    ...
    by_cases hx : g.cycleOf x = 1
    · rw [hθ_1 _ x]
      simp only [u, v, Equiv.Perm.subtypePerm_apply]
  ...
  · rintro ⟨⟨u, v⟩, h⟩
    ...
    intro c hc
    suffices hc' : ∀ (x : α), z x ∈ c.support ↔ x ∈ c.support by
      use hc'
      suffices Equiv.Perm.ofSubtype (Equiv.Perm.subtypePerm z hc') = v ⟨c, hc⟩ by
        rw [this]
        simp
      ext x
      by_cases hx : x ∈ c.support
...
theorem hψ_range_card' (g : Equiv.Perm α) :
  ...
  let u : (Set.range (θ g)) → (φ g).ker := fun ⟨z, hz⟩ => by
    ...
    rw [← hu']
    exact u.prop
  suffices Function.Bijective u by
    exact Fintype.card_of_bijective this
  constructor
  · -- injectivity
    rintro ⟨z, hz⟩ ⟨w, hw⟩ hzw
    simpa only [u, Subtype.mk_eq_mk, MulEquiv.apply_eq_iff_eq] using hzw
  · -- surjectivity
    rintro ⟨w, hw⟩
...
theorem Equiv.Perm.card_fixedBy (g : Equiv.Perm α) :
  ...
  simp only [MulAction.mem_fixedBy, Equiv.Perm.smul_def, Finset.mem_filter, Finset.mem_univ,
    true_and, Finset.mem_compl, Equiv.Perm.mem_support, Classical.not_not]
...
theorem Equiv.Perm.conj_class_card_mul_eq (g : Equiv.Perm α) :
    Nat.card ({h : Equiv.Perm α | IsConj g h} : Set _) *
...
theorem Equiv.Perm.card_of_cycleType_mul_eq (m : Multiset ℕ) :
  ...
  · -- nonempty case
    ...
    suffices (Finset.univ.filter fun h : Equiv.Perm α => h.cycleType = m) =
      ...
      rw [this, ← Equiv.Perm.conj_class_card_mul_eq g]
...
theorem Equiv.Perm.card_of_cycleType (m : Multiset ℕ) :
  ...
  · -- nonempty case
    ...
    rw [Nat.div_eq_of_eq_mul_left _]
    apply congr_arg₂
    · let _ : Fintype (Equiv.Perm α) := inferInstance
      let _ : Fintype {h | IsConj g h} := Fintype.ofFinite _
...
lemma θ_apply_fst
  ...
  · rw [hθ_2 _ x ⟨g.cycleOf x, hx⟩ rfl]
    ...
    exact hx
  · rw [Equiv.Perm.cycleOf_mem_cycleFactorsFinset_iff, Equiv.Perm.notMem_support] at hx
...
lemma θ_apply_single
  ...
  · rw [hθ_2 _ x ⟨g.cycleOf x, hx⟩ rfl]
    ...
    · simp only [ne_eq]
      ...
      obtain ⟨m, hm⟩ := vc.prop
      dsimp at hm
      rw [← hm, ← Equiv.Perm.notMem_support]
      intro hx'
      apply hc
      apply symm
...
theorem odd_of_mem_kerφ
  ...
  suffices Equiv.Perm.sign c = 1 by
    ...
    · rw [← Nat.not_even_iff_odd, Function.comp_apply]
      rw [← neg_one_pow_eq_one_iff_even (R := ℤˣ) (by norm_num), this, ← Units.eq_iff]
      norm_num
    · rw [Equiv.Perm.mem_cycleFactorsFinset_iff] at hc
      exact hc.left
  ...
  suffices hτ : τ ∈ OnCycleFactors.Iφ g by
    ...
    have hk_apply :
      ...
      suffices ∀ m n : ℕ, Commute (k ^ m) (g ^ n) by
        ...
        · simp only [pow_zero, Equiv.Perm.coe_one, id]
        ...
        · -- k.support = c.support ∪ d.support
          ...
          · intro hx
            ...
            have hxcx : x ∈ (cx : Equiv.Perm α).support := by
              rw [Equiv.Perm.SameCycle.eq_cycleOf cx
                (hcx (a cx) (a.mem_support_self cx)) (a.mem_support_self cx),
            ...
            · obtain ⟨n, _, hnx⟩ := (hcx (a cx) (a.mem_support_self cx)).exists_pow_eq'
          ...
          suffices hx' : Equiv.Perm.cycleOf g x = c ∨ Equiv.Perm.cycleOf g x = d by
            ...
            obtain ⟨n, _, hnx⟩ := Equiv.Perm.SameCycle.exists_pow_eq'
              (hcx (a cx) (a.mem_support_self cx))
            specialize hk_apply cx 1
            simp only [pow_one] at hk_apply
            ...
            have : ¬Equiv.Perm.Disjoint (cx : Equiv.Perm α) (τ cx : Equiv.Perm α) := by
              ...
              use a cx
              apply And.intro (a.mem_support_self cx)
              rw [← haτcx_eq_acx]; exact a.mem_support_self (τ cx)
            have this' := (Set.Pairwise.eq
              g.cycleFactorsFinset_pairwise_disjoint cx.prop (τ cx).prop this).symm
            rw [Subtype.coe_inj] at this'
            rw [← Equiv.Perm.notMem_support] at this'
    ...
    · -- orderOf k = 2
      ...
      · -- obtain ⟨cx, hcx, hcx'⟩ := hsame_cycle x hx,
        ...
        obtain ⟨n, _, rfl⟩ := (hcx (a cx) (a.mem_support_self cx)).exists_pow_eq'
        convert hk_apply cx 2 n
        suffices hτ2 : τ ^ 2 = 1 by
          rw [hτ2, Equiv.Perm.coe_one, id]
    ...
    apply hm'
    specialize hk2 ⟨c, hc⟩
    simpa only [hk, map_one, one_smul, Equiv.swap_apply_left, τ] using hk2
  · intro x
    by_cases hx : x = ⟨c, hc⟩
...
theorem kerφ_le_alternating_iff :
  ...
  · rintro ⟨h_odd, h_fixed, h_count⟩ x hx
    suffices hx' : x ∈ Set.range (θ g) by
    ...
    rw [← Multiset.nodup_iff_count_le_one, Equiv.Perm.cycleType_def,
      ...
      at h_count
    rw [Equiv.Perm.coe_one, id, ← Subtype.coe_inj]
